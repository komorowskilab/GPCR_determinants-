...
def match_fasta_id(distance_id, aligned_sequences):
    """
    Safely match a distance table ID to a FASTA sequence ID.

    Strategy:
      1. Exact match first (handles simple IDs like 'M45', 'seq10', etc.)
      2. Prefix match, but only when the remainder starts with '_' or '/'
         e.g. 'xp_076580365_temnothorax' -> 'xp_076580365'  (remainder: '_temnothorax') OK
              'm45'                       -> 'm4'             (remainder: '5')           REJECTED

    Parameters
    ----------
    distance_id      : str  — already lowercased caller side
    aligned_sequences: dict — keys are FASTA IDs (lowercased)
    ...
    """
    # 1. Exact match
    if distance_id in aligned_sequences:
        return distance_id

    # 2. Prefix match only when suffix begins with a non-alphanumeric separator
    for fid in aligned_sequences:
        if distance_id.startswith(fid):
            remainder = distance_id[len(fid):]
            if remainder and re.match(r'^[_/]', remainder):
                return fid
        ...
        with open(tsv_file, 'r') as f:
            for line in f:
                ...
                if line and not line.startswith('#'):
                    parts = line.split('\t')
                    if len(parts) >= 2:
                        seq_id = parts[0].split('_')[0].lower()
                        seq = parts[1]
...
def align_distances_to_positions(distance_df, aligned_sequences, aligned_length=135):
    ...
    for idx, row in distance_df.iterrows():
        ...
        matched_fasta_id = match_fasta_id(distance_id, aligned_sequences)
...
def create_detailed_mapping_all_species(distance_df, aligned_sequences):
    ...
    for idx, row in distance_df.iterrows():
        ...
        matched_fasta_id = match_fasta_id(seq_id, aligned_sequences)
...
def print_aligned_table_example(aligned_df, aligned_sequences, n_sequences=5):
    ...
    for the first few sequences in aligned_df.
    """
    for idx in range(min(n_sequences, len(aligned_df))):
        ...
        aligned_seq = aligned_sequences[matched_fasta_id]
        distances = row[1:].values   # all position columns
...
if __name__ == "__main__":
    ...
    distance_df = pd.read_csv('distance_table_v5.tsv', sep='\t', index_col=0)   # Adjust filename
    ...
    aligned_sequences = parse_aligned_sequences('aligned_without_validation.fa')   # Adjust filename
    ...
    # Print first 5 sequences as a visual check
